    ...
    <div>
        <div class="paginator pagination example" v-show="data.to > 1">
            <div class="pagination pg-blue">
                ...
                <button class="page-item" v-if="(data.current_page - 1) != 1 && (data.current_page) != 1"
                    @click="changePage(data.current_page - 1)">
                    {{ data.current_page - 1 }}
                </button>
                ...
                <button class="page-item" v-if="data.current_page != data.last_page && data.last_page"
                    @click="changePage(data.last_page)">
                    {{ data.last_page }}
                </button>
                <button class="page-item" :disabled="data.current_page == data.last_page || loading"
                    :class="{ 'disabled': data.current_page == data.last_page || loading }"
                    @click="changePage(data.current_page + 1)">
